  ...
  <div class>
    <div class="shadow-xl bg-white rounded-2xl hover:shadow-2xl">
      <div class="grid md:grid-cols-3 gap-0">
        <div
          class="md:col-span-1 bg-primary-300 rounded-t-2xl md:rounded-tr-none md:rounded-tl-2xl relative"
        >
          <img
        ...
        <div class="md:col-span-2 py-6 px-4">
          ...
          <router-link :to="{ name: 'ProjectDetails', params: { slug: project.slug } }">
            <a class="cursor-pointer hover:text-primary-700" target="_blank">
              <div class="font-semibold text-xl">{{ project?.title }}</div>
            </a>
          </router-link>
          <div class="text-base text-gray-500" v-if="project?.categories">{{ project?.location }}</div>

          <div class="text-2xl font-black text-primary-500 my-2">{{ toCurrency(project?.fee) }}</div>
      ...
      <div class="text-sm uppercase" v-if="progress > 0">
        <ProgressBar :value="progress" class="rounded-b-2xl h-24 md:h-6">
          <div
            class="flex flex-wrap flex-col-reverse justify-center md:flex-row md:justify-between items-center font-medium pt-1 pb-2 px-2"
          >
            <div>
